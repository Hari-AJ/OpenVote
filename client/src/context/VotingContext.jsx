...
export const TransactionsProvider = ({ children }) => {
    ...
    try {
      if (ethereum) {
        ...
        for (let i = 0; i < cancount; i++) {
          ...
          temp.push({ partyName: candidate[0], candidateName: candidate[1] ,votes: candidate[2].toNumber(), partyLogo: candidate[3], candidateLogo: candidate[4]  });
        }
        setCandidates(temp);
      } else {
  ...
  const setElectionStart= ()=>{
    console.log('hit');
    setElection(1);

  }

  const setElectionEnd= ()=>{
    console.log('hit');
    setElection(2);
